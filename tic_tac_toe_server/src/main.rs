...
async fn join_game_handler(
    ...
    if let Err(e) = state.tx.send((req.game_id.clone(), game.clone())) {
        error!("Failed to broadcast updated game state: {:?}", e);
    }

    // Notify waiting state if only one player is present
    if game.players.len() == 1 {
        info!(
            "Game {} is waiting for the second player to join.",
            req.game_id
        );
    } else if game.players.len() == 2 {
        info!("Game {} is ready to start with both players.", req.game_id);
    }

    Json(Ok(game.players.last().unwrap().clone()))
}
